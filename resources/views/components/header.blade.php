    ...
    <nav class="bg-white dark:bg-gray-900 w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
            <a href="{{route('home')}}" class="flex items-center space-x-3 rtl:space-x-reverse">
                <span
                    class="self-center text-lg font-semibold whitespace-nowrap text-blue-700 dark:text-white ">VaccineFlow</span>
            </a>
            @if (auth()->check())
                <div class="flex gap-4 md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
                    <a href="{{route('profile')}}" type="button"
                        class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">{{auth()->user()->name}}</a>
            ...
            @else
                <div class="flex gap-4 md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
                    ...
                    <a href="{{route('registration')}}" type="button"
                        class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Registration</a>
                </div>
            @endif
